...
export const cleanMetrics = (font: OpenTypeFont): void => {
  for (let i = 0; i < font.glyphs.length; i++) {
    ...
    // --- PRESERVE NON-LETTERS (Punctuation, Numbers, Symbols) ---
    // If the glyph is NOT a Letter (\p{L}), we skip cleaning its metrics.
    // This ensures punctuation keeps its default spacing.
    let preserve = false;
    
    if (glyph.unicode) {
        try {
            const char = String.fromCodePoint(glyph.unicode);
            // Regex checks if it is a unicode LETTER.
            // If it is NOT a letter, we preserve it.
            if (!/\p{L}/u.test(char)) {
                preserve = true;
            }
        } catch (e) {
            preserve = true; // Safe fallback
        }
    } else {
        // Glyphs without unicode are likely internal alternates or .notdef, preserve them.
        preserve = true;
    }
